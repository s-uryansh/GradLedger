      ...
      <div className="mt-24 max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {users
            ...
            .map((u) => (
              <div key={u._id} className="relative group cursor-pointer" onClick={() => router.push(`/profile/${u._id}`)}>
                <ProfileCard
                  name={u.fullName}
                ...
                />
                <button
                  onClick={(e) => { e.stopPropagation(); openChat(u._id); }}
                  className="absolute bottom-3 right-3 bg-blue-600 text-white px-2 py-1 rounded-md opacity-0 group-hover:opacity-100 transition"
                >
                  Chat
                </button>
              </div>
        ...
        {(page < totalPages) && users.length > 0 && (
          <button onClick={() => setPage((p) => p + 1)} className="w-full mt-8 py-3 bg-white/10 rounded-lg text-white hover:bg-white/20">Load More</button>
        )}
      </div>
    </div>
  );
